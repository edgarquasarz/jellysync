...
describe('analyzeDiff', () => {
  ...
  function analyzeDiff(
    ...
    new: MockServerTrack[];
    metadataChanged: MockServerTrack[];
    unchanged: MockServerTrack[];
    removed: string[];
  } {
    const result = {
  ...
  it('marks server track as new when not in synced tracks', () => {
    const serverTracks = [
      { id: 'track-1', name: 'New Track', path: '/t1.mp3', metadataHash: 'abc' },
    ];
    const syncedTracks = new Map<string, MockSyncedTrack>();

    const result = analyzeDiff(serverTracks, syncedTracks, {
      coverArtMode: 'embed',
      bitrate: '192k',
      convertToMp3: false,
    });
  ...
  it('marks server track as unchanged when metadata hash matches', () => {
    const serverTracks = [
      { id: 'track-1', name: 'Track', path: '/t1.mp3', metadataHash: 'abc123' },
    ];
  ...
  it('marks server track as metadata_changed when hash differs', () => {
    const serverTracks = [
      { id: 'track-1', name: 'Track Updated', path: '/t1.mp3', metadataHash: 'xyz789' },
    ];
    const syncedTracks = new Map([
      [
        'track-1',
    ...
    const syncedTracks = new Map([
      [
        'track-changed',
        {
          ...
          destinationPath: '/changed.mp3',
          fileSize: 100,
          metadataHash: 'hold',
          coverArtMode: 'embed',
          encodedBitrate: '192k',
          serverPath: '/changed.mp3',
        },
      ],
      [
        'track-unchanged',
        {
          trackId: 'track-unchanged',
          destinationPath: '/same.mp3',
          fileSize: 100,
          metadataHash: 'hsame',
          coverArtMode: 'embed',
          encodedBitrate: '192k',
      ...
      [
        'track-deleted',
        {
          ...
          destinationPath: '/deleted.mp3',
          fileSize: 100,
          metadataHash: 'hdel',
          coverArtMode: 'embed',
          encodedBitrate: '192k',
          serverPath: '/deleted.mp3',
        },
      ],
    ]);
  ...
  it('detects cover_art_changed when mode differs', () => {
    ...
    const syncedTracks = new Map([
      [
        'track-1',
        {
          ...
          fileSize: 100,
          metadataHash: 'h1',
          coverArtMode: 'off', // was synced with cover art OFF
          encodedBitrate: '192k',
          serverPath: '/t1.mp3',
        },
      ],
    ...
    const coverArtChanged =
      synced.metadataHash === _serverTracks[0].metadataHash &&
  ...
  it('detects bitrate_changed when encoded_bitrate differs from current option', () => {
    ...
    const bitrateChanged =
      options.convertToMp3 === true && synced.encodedBitrate !== options.bitrate;
  ...
  it('detects path_changed when album is renamed on server', () => {
    ...
    const serverTracks = [
      { id: 'track-1', name: 'Track', path: '/Artist/NewAlbum/track.mp3', metadataHash: 'abc123' },
    ];
    const syncedTracks = new Map([
      [
        'track-1',
