...
Then('library items or an empty state should be shown', async function (this: ICustomWorld) {
  ...
  const hasItems = await this.page!.locator('[data-testid="library-item"]')
    .first()
    .isVisible()
    .catch(() => false);
  const hasEmpty = await this.page!.locator('[data-testid="library-empty"]')
    .isVisible()
    .catch(() => false);
  const hasLoading = await this.page!.locator('[data-testid="library-loading"]')
    .isVisible()
    .catch(() => false);
